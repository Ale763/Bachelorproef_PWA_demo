...
async function t()
{
	...
	// return await caches.match(new Request("/img/horse.png"), { runtime });
	/* return CacheLib.getFromCache(RUNTIME, "/img/horse.png");
	if (SW_VERSION === 0)
		return CacheLib.getFromCache(RUNTIME, '/img/cat.png');
	else
		return CacheLib.getFromCache(RUNTIME, '/img/dog.png'); */
}
...
async function update(event)
{
	...
	{
		console.log("Sending skipWaiting")
		registration.waiting.postMessage({ action: 'skipWaiting' });
		let response = await fetch(event.request);
		let newResponse = new Response("Loading...", { headers: { "Refresh": "0" } });
		return newResponse;
